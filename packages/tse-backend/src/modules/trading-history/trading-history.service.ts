...
export class TradingHistoryService {
  ...
  async getUserTradingHistory(params: GetUserTradingHistoryParamsCommand, queries: GetUserTradingHistoryQueryCommand) {
    const { startDate, endDate, exchangeName, symbol, type, status, side } = queries;
    ...
    const query: FindManyOptions = {
      where: {
        ...
        ...(startDate && endDate && { createdAt: Between(new Date(startDate), new Date(endDate)) }),
